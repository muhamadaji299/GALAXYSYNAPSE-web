...
const LoginPage = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await fetch('https://chat-api-production-0347.up.railway.app/api/auth/login', {
        method: 'POST',
        headers: {
    ...
    <div className="relative min-h-screen flex items-center justify-center px-4 py-20">
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400">Enter your credentials to access the cosmos.</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-white text-black font-semibold py-3 rounded-lg hover:bg-gray-200 transition-all active:scale-[0.98] flex items-center justify-center gap-2"
          >
